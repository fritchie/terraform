...
resource "libvirt_volume" "base_vol" {
	...
	format = "qcow2"
	source = "/mnt/libvirt/bionic-server-cloudimg-amd64.img"
	pool   = "default"
}

data "template_file" "user_data" {
	count    = var.hosts
	template = file("${path.module}/cloud_init_u18_${count.index + 1}.cfg")
}

resource "libvirt_cloudinit_disk" "cloud_init" {
	count     = var.hosts
	name      = "cloudinit_u18_${count.index + 1}-ubuntu.iso"
	user_data = data.template_file.user_data[count.index].rendered
	pool      = "default"
}

resource "libvirt_domain" "u18" {
	count     = var.hosts
	name      = format(var.hostname_format, count.index + 1)
	memory    = "8096"
	vcpu      = 8
